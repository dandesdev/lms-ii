...
export function UsageCard() {
  ...
  const totalSeg = usage.total_bytes || 1;

  return (
    <Card className={cn(level !== "normal" && "border-[#8a5a10]/40")}>
      ...
      <CardContent className="space-y-3">
        <div>
          <div className="mb-1 flex justify-between font-mono text-[11px]">
            <span>{formatBytes(usage.total_bytes)} of {formatBytes(plan.quota_bytes)}</span>
            <span>{percent}%</span>
          </div>
